...
public abstract class Command_java {
    ...
    public void reloadAudioFiles() {
        if (!getCategory().equals(Category_java.AUDIO)) return;
    ...
    protected String getRandomTrack() {
        if (!getCategory().equals(Category_java.AUDIO)) return null;
        return audioFiles[SpoopyUtils_java.random.nextInt(audioFiles.length)];
    }
    ...
    protected GuildMusicManager getMusicManager(Guild guild) {
        return SpoopyUtils_java.audio.getMusicManager(guild);
    }

    protected boolean preAudioChecks(GuildMessageReceivedEvent event) {
        if (!event.getMember().getVoiceState().inVoiceChannel()) {
            sendEmbed(event, EmbedUtils_java.embedMessage("Please join a voice channel first"));
            return false;
        }
        ...
        try {
            ...
            if (e.getPermission() == Permission.VOICE_CONNECT) {
                sendMsg(event, EmbedUtils_java.embedMessage(String.format("I don't have permission to join `%s`", event.getMember().getVoiceState().getChannel().getName())));
            } else {
                sendMsg(event, EmbedUtils_java.embedMessage(String.format("Error while joining channel `%s`: %s"
