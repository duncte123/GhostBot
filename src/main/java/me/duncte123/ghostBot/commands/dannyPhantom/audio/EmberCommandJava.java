...
public class EmberCommandJava extends Command_java {
    @Override
    public void execute(String invoke, String[] args, GuildMessageReceivedEvent event) {
        if (preAudioChecks(event)) {
            sendMsg(event, "Selected track: _Ember McLain - Remember_");
            SpoopyUtils_java.audio.loadAndPlay(getMusicManager(event.getGuild()), event.getChannel(), "wBMOc24_aIw", false);
        }
    }
