...
import me.duncte123.ghostBot.objects.tumblr.TumblrDialogue_java;
import me.duncte123.ghostBot.objects.tumblr.TumblrPost_java;
import me.duncte123.ghostBot.utils.EmbedUtils_java;
import me.duncte123.ghostBot.utils.MessageUtils_java;
...
public class QuotesCommandJava extends Command_java {
    ...
    private final List<TumblrPost_java> tumblrPosts = new ArrayList<>();
    private final Map<String, Integer> indexes = new HashMap<>();

    private int oldCount = 0;

    public QuotesCommandJava() {
        reloadQuotes();
    }
    ...
    public void execute(String invoke, String[] args, GuildMessageReceivedEvent event) {
        ...
        if (args.length > 0) {
            if ("reload".equals(args[0])) {
                ...
                if (event.getAuthor().getId().equals(Variables_java.OWNER_ID)) {
                    reloadQuotes();
                    new Thread(() ->
                            MessageUtils_java.sendMsg(event, messages[0], success -> {
                    ...
                    MessageUtils_java.sendMsg(event, "Only the bot owner can reload quotes");
                }
            } else if ("total".equals(args[0])) {
                MessageUtils_java.sendMsg(event, "There are a total of " + tumblrPosts.size() + " quotes in the system at the moment");
            }
            return;
        }
        ...
        TumblrPost_java post = tumblrPosts.get(index);
        ...
        switch (type) {
            case "chat":
                List<TumblrDialogue_java> dialogue = post.dialogue;
                dialogue.forEach(
        ...
        MessageUtils_java.sendEmbed(event, eb.build());
    }
    ...
    private void reloadQuotes() {
        ...
        for (String type : types) {
            ...
            String url = String.format(
                    ...
                    SpoopyUtils_java.config.getString("api.tumblr", "API_KEY"),
                    type
            );
            WebUtils.ins.getAson(url).async(json -> {
                ...
                for (int i = 0; i <= total; i += 20) {
                    WebUtils.ins.getAson(url + "&offset=" + (i > 1 ? i + 1 : 1)).async(j -> {
                        ...
                        List<TumblrPost_java> posts = Ason.deserializeList(fetched, TumblrPost_java.class);
                        List<TumblrPost_java> filteredPosts = posts.stream().filter(post -> !badPostIds.contains(post.id)).collect(Collectors.toList());
                        tumblrPosts.addAll(
                                filteredPosts
                        );
