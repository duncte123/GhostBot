...
class QuotesCommand extends Command {
    ...
    void execute(String invoke, String[] args, GuildMessageReceivedEvent event) {
        ...
        String gid = event.guild.id
        if (!indexes.containsKey(gid) || indexes[gid] >= tumblrPosts.size()) {
            indexes[gid] = 0
        }
            ...
            WebUtils.ins.getAson(url).async({ json ->
                ...
                for (int i = 0; i <= total; i += 20) {
                    WebUtils.ins.getAson(url + "&offset=" + (i > 1 ? i + 1 : 1)).async({ j ->
                        ...
                        List<TumblrPost> posts = Ason.deserializeList(fetched, TumblrPost.class, true)
                        List<TumblrPost> filteredPosts = posts.stream().filter { post -> !badPostIds.contains(post.id) }.collect().toList() as List<TumblrPost>
                        tumblrPosts.addAll(
                                filteredPosts
                        )
