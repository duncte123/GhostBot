...
class DrakeCommand extends ImageBase {
    ...
    void execute(CommandEvent event) {
        ...
        if (!event.fromSlack) {
            ...
            if (!jdaEvent.guild.selfMember.hasPermission(jdaEvent.channel, Permission.MESSAGE_ATTACH_FILES)) {
                sendMessage(event, 'I need permission to upload files to this channel')
                return
            }
        }
        ...
        if (args.length == 0) {
            sendMessage(event, "Missing arguments, usage: `$Variables.PREFIX$name <top text>|<bottom text>`")
            return
        }
